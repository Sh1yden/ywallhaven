...
- [ ] Добавить информирование пользователя о ошибках приложения и сервиса wallhaven.
- [ ] Добавить выбор сборки релиза или пре релиза. (Предпологается пока такой механизм: 
`если предыдущая версия уже с тем же номером что и новая то это пре релиз` 
и `если версия новая отличается значит это релиз`. 
Пример: `0.9.1->0.9.2(номера после 0 одинаковые значит 092 это пре релиз)` 
и `0.9.2->0.10.0`(0100 это уже релизная версия).
    - [ ] Обдумать структуру релизов и пре-релизов...
